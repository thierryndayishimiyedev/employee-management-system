...
const resourceConfig = {
  ...
  reports: {
    ...
    form: [
      ...
      ['report_range_preset', 'Quick period', 'reportpreset', false, 'createOnly'],
      ['report_date', 'Date inside the report period', 'date', true],
      ['period_start', 'Custom period start', 'date', true, 'customOnly'],
      ['period_end', 'Custom period end', 'date', true, 'customOnly'],
    ...
    columns: [
      ...
      ['Correction / deletion note', (item) => item.status === 'DELETE_REQUESTED' ? item.deletion_request_reason || 'Deletion requested' : item.status === 'CHANGES_REQUESTED' ? item.manager_decision_reason || item.owner_decision_reason || 'Changes requested' : '—'],
    ],
    actions: [
      {
      ...
      { label: 'Download report PDF', icon: Download, roles: ['ACCOUNTANT', 'MANAGER', 'OWNER'], download: true },
      {
        label: 'Manager Approve',
        icon: CheckCircle2,
        roles: ['MANAGER'],
      ...
      },
      {
        ...
        commentsPrompt: 'Explain the corrections needed for the accountant:',
        run: (item, comments) => api.put(`/reports/${item.report_id}/review`, { decision: 'reject', comments }),
      },
      {
        label: 'Final Approve',
        icon: CheckCircle2,
      ...
      {
        label: 'Request deletion', icon: Trash2, roles: ['ACCOUNTANT'], show: (item) => item.status !== 'DELETE_REQUESTED', commentsPrompt: 'Why should this report be deleted? This will be sent to the manager:', confirm: 'Send a deletion request to the manager?', run: (item, reason) => api.put(`/reports/${item.report_id}/delete-request`, { reason }),
      },
      { label: 'Approve deletion', icon: Trash2, roles: ['MANAGER'], show: (item) => item.status === 'DELETE_REQUESTED', confirm: 'Delete this report permanently? This cannot be undone.', run: (item) => api.put(`/reports/${item.report_id}/delete-review`, { decision: 'approve' }) },
      { label: 'Keep report', icon: XCircle, roles: ['MANAGER'], show: (item) => item.status === 'DELETE_REQUESTED', commentsPrompt: 'Why should this report be kept?', run: (item, comments) => api.put(`/reports/${item.report_id}/delete-review`, { decision: 'reject', comments }) },
    ],
  },
  payrolls: {
    title: 'Payroll',
...
export default function ManagementPage({ resource }) {
  ...
  const setField = (field, value) => {
    if (config.endpoint === '/reports' && field === 'report_range_preset' && value) {
      const end = new Date(`${today}T00:00:00`)
      const start = new Date(end)
      const weeks = Number(value)
      start.setDate(start.getDate() - ((start.getDay() || 7) - 1) - (weeks * 7))
      end.setTime(start.getTime())
      end.setDate(end.getDate() + (weeks * 7) - 1)
      const iso = (date) => date.toISOString().slice(0, 10)
      setForm((current) => ({ ...current, report_range_preset: value, report_type: 'CUSTOM', period_start: iso(start), period_end: iso(end), report_date: iso(end) }))
      return
    }
  ...
  const payload = () => {
    ...
    delete next.report_range_preset
  ...
  const runAction = async (action, item) => {
    ...
    let comments
    if (action.commentsPrompt) {
      comments = window.prompt(action.commentsPrompt)
      if (comments === null || !comments.trim()) return
    }
    try {
      if (action.download) {
        const start = item.period_start || item.report_date
        const end = item.period_end || item.report_date
        const response = await api.get('/downloads/reports/pdf', { params: { start, end }, responseType: 'blob' })
        const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }))
        const link = document.createElement('a'); link.href = url; link.setAttribute('download', `operations-report-${start}-to-${end}.pdf`); document.body.appendChild(link); link.click(); link.remove(); window.URL.revokeObjectURL(url)
        toast.success('Report PDF downloaded.')
        return
      }
      const result = await action.run(item, comments)
            ...
            <form onSubmit={saveItem} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-1">
                {config.form.map(([field, label, type, required, mode]) => {
                  ...
                  if (mode === 'customOnly' && form.report_type !== 'CUSTOM') return null
...
function Field({ label, type, value, onChange, required, related }) {
  ...
  if (type === 'reportpreset') {
    return (
      <SelectBase label={label} value={value} onChange={onChange} required={required}>
        <option value="">Choose manually / no preset</option>
        <option value="1">Previous 1 complete week</option>
        <option value="2">Previous 2 complete weeks</option>
        <option value="3">Previous 3 complete weeks</option>
      </SelectBase>
    )
  }
