...
const createBatch = async (req,res) => { try { res.status(201).json({success:true,data:await service.recordConsumptionsBatch(req.body||{},req.user)}); } catch(err) { res.status(400).json({success:false,message:err.message}); } };
...
const capacity = async (req,res) => { try { res.json({success:true,data:await service.getConsumptionCapacity(req.params.employeeId,req.user)}); } catch(err) { res.status(400).json({success:false,message:err.message}); } };
...
module.exports={create,createBatch,list,capacity,approve,changes,pay,payAll};
